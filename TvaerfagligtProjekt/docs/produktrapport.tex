...
checkbox og wordwrap-tekst. Det er renderings-agnostisk og kan anvendes med
...
deterministiske byg med lav overhead (se Referencer \cite{tup-algorithm}).

\newpage
\section{Arkitektur}
Nedenstående figur viser den overordnede systemarkitektur.
(se Figure \ref{fig:high-level-architecture}).
...
\subsection{App}
Appen er delt op i fire primære lag (se Figure \ref{fig:app-architecture}).

Brugergrænsefladen præsenterer data og interaktioner for brugeren. Den
indeholder skærmbilleder, UI‑komponenter, inputvalidering og præsentationslogik.
UI’et kommunikerer med databehandlingslaget via veldefinerede API’er for at
holde visning og forretningslogik adskilt.

Databehandlingslaget rummer forretningslogik, tilstandshåndtering,
inputbearbejdning og validering. Det håndterer synkronisering, konflikthåndtering
og træffer beslutning om, hvilke operationer der skal udføres på data.

Datalagringslaget er ansvarligt for vedvarende opbevaring. Det tilbyder
abstrakte CRUD‑operationer, transaktionsstyring og sikkerhed, så data kan
tilgås og beskyttes ensartet.

Dataudvekslingslaget står for netværkskommunikation med andre enheder og
signaling‑servere. Det understøtter peer‑to‑peer‑kommunikation — herunder
enhedsopdagelse via signaling, direkte meddelelsesudveksling, forbindelsesstyring,
genopretning ved fejl, sikre forbindelser, retry‑logik og batching.

\begin{figure}[H]
  \centering
  \includegraphics[width=.4\textwidth]{app-architecture.png}
  \caption{App Arkitektur}
  \label{fig:app-architecture}
  \centering
\end{figure}

\subsection{Database}
Der benyttes en relationel database med to tabeller: device og note. Note er
en weak entity knyttet til device (se Figure \ref{fig:er-diagram}).

Device-tabellen indeholder den givne enhed samt alle kendte enheder, som data
udveksles mellem. Note-tabellen kan kun eksistere i relation til sin
tilknyttede device.
...
\section{Opsætning}
Projektet er tilgængelig på Github (se Referencer \cite{marc8193_tvaerfagligtprojekt}).

Forudsætninger for at bygge og installere projektet er Tup build system, Android
SDK, NDK og platform-tools.

\begin{lstlisting}[language=bash]
$ tup init
$ tup
$ adb install build/note.apk
\end{lstlisting}
