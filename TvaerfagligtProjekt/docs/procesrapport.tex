...
\newcommand{\supervisor}{Lars Thise Pedersen}
...
\begin{center}  
  ...
  {\large \textbf{Fremlæggelsesdato:} 19 el. 20/03-2026}
  \vspace*{\fill}
...
Det anbefales, at læseren først læser procesrapporten og derefter produktrapporten.
...
opdelt i fem underafsnit: Brugergrænseflade, Databehandling, Datalagring, Dataudveksling og Byggesystem.
...
og grænsefladen til den decentraliseret dataudveksling mellem enheder.
...
retained-mode layoutsystemer (se Referencer \cite{smith2024_proving}).
...
fleste grafiske brugergrænseflader (se Listing \ref{lst:renderer}).
...
gør det nemt at allokere eller frigive dele af bufferen samt hele bufferen (se Figure \ref{fig:arena-allocate}).
...
overskuelig mental model over den allokerede hukommelse (se Figure \ref{fig:stack-allocate}).
...
som kan skalere, efterhånden som der tilføjes flere enheder og noter (se Figure \ref{fig:er-diagram}).

\begin{figure}[H]
  \centering
  \includegraphics[width=.75\textwidth]{er-diagram.png}
  \caption{ER-diagram}
  \label{fig:er-diagram}
  \centering
\end{figure}

\subsection{Dataudveksling}
Der benyttes peer-to-peer-kommunikation mellem enheder, så hver enhed både kan
initiere og modtage forbindelser direkte til andre enheder uden en central server.
Dataudvekslingslaget håndterer enhedsopdagelse via en signaling-server, direkte
meddelelsesudveksling, forbindelsesstyring og genopretning ved fejl, hvilket øger
skalerbarhed og redundans.

\subsection{Byggesystem}
Der anvendes et filbaseret byggesystem, som muliggør hurtige iterationer ved  
at registrere filændringer og bygge en rettet acyklisk graf (DAG) over  
afhængigheder. Systemet evaluerer DAG'en og udfører kun nødvendige  
kommandoer for at opdatere afhængige filer. Opdateringerne sker med minimal  
overhead, idet byggesystemet bruger effektive byggealgoritmer, der undgår  
unødvendigt arbejde.

\section{Begrænsninger}
Hvis der havde været mere tid til rådighed, kunne projektet have udviklet sig til et mere færdigt produkt.
Da projektet primært bygger på skræddersyede løsninger, har udviklingstiden været lang,
hvilket har resulteret i begrænset funktionalitet.

\section{Realiseret tidsplan}
Den oprindelige tidsplan har været udfordrende at følge, da det er
vanskeligt at planlægge med de fragmenterede ressourcer, der har været
til rådighed. Der er primært to årsager til, at tidsplanen ikke er
blevet overholdt: optimisme og momentum.

Tidsplanens kompaktheden har bidraget til problemerne, hvilket
har vanskeliggjort en realistisk implementering af arbejdsopgaverne. 
Desuden har tabet af momentum haft en betydelig indflydelse, da der
ikke har været mulighed for at allokere hele eller størstedelen af
arbejdsugen til projektet. Dette har resulteret i forsinkelser og
manglende fremskridt, hvilket har påvirket den samlede kvalitet af det
færdige produkt. En mere fleksibel og realistisk plan kunne have
hjulpet med at imødekomme disse udfordringer bedre.

\section{Konklusion}
Det er muligt at skabe produkter, som er uafhængige af centrale
platforme, uden at gå på kompromis med tilgængelighed og
brugervenlighed. Dog kommer disse løsninger med nogle ukendte
udfordringer, da der ikke er kutyme for at skabe applikationer med
denne arkitektur.

Ved at anvende et programmeringssprog, som er en central byggesten i
alle moderne systemer, kan man udvikle applikationer, som næsten er
platformagnostiske. Dette har faciliteret udvikling med hurtige
iterationer og integration på tværs af forskellige platforme, hvor man
ønsker at understøtte decentralisering og digitalt ejerskab,
uden at gå på kompromis med tilgængelighed og brugervenlighed.
